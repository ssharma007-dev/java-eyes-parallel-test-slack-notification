...
function makeTunnelClientWorker({ settings, logger: defaultLogger, }) {
    const logger = (0, logger_1.makeLogger)({
        ...
        handler: { type: 'rolling file', name: 'tunnel-client', dirname: utils_1.LOG_DIRNAME },
    ...
    if (consecutiveFailures === 0) {
        logger.console.log(chalk_1.default.bold(`execution-grid-tunnel logs are written to ${path.resolve(utils_1.LOG_DIRNAME)}`));
    }
    process.on('unhandledRejection', error => {
        const errorMessage = `Unhandled rejection terminating the process with consecutive failures: ${consecutiveFailures}. Error: ${error}`;
        logger.console.error(errorMessage);
        restartTunnelClientWorker({ settings, logger });
    });
    process.on('uncaughtException', error => {
        const errorMessage = `Uncaught error terminating the process with consecutive failures: ${consecutiveFailures}. Error: ${error}`;
        logger.console.error(errorMessage);
        restartTunnelClientWorker({ settings, logger });
    });
...
async function runSession({ settings, logger }) {
    ...
    return new Promise((resolve, reject) => {
        socket.on('TunnelClient.close', async (reason) => {
            ...
            // save the first error message. ignore consecutive error messages as it may be different from the initial error
            if (consecutiveFailures === 0)
                errorMessage = (reason === null || reason === void 0 ? void 0 : reason.toString()) || 'Worker is going to be closed due to unknown reason';
            // this may fail due to network conditions so we try on every socket close with the original message
            void (0, req_logs_1.reportToCoralogix)({
                settings: {
                    level: 0,
                    eventType: 'TunnelClientSDK',
                    event: {
                        type: 'Error',
                        ...
                        envInfo: (0, metrics_1.extractEnvInfo)(),
                    },
                },
        ...
        socket.on('error', async (reason) => {
            ...
            // save the first error message. ignore consecutive error messages as it may be different from the initial error
            if (consecutiveFailures === 0)
                errorMessage = (reason === null || reason === void 0 ? void 0 : reason.toString()) || 'Session: Encountered fatal error. Cleaning up and signaling exit';
            // this may fail due to network conditions so we try on every socket error with the original message
            void (0, req_logs_1.reportToCoralogix)({
                settings: {
                    level: 0,
                    eventType: 'TunnelClientSDK',
